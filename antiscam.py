...
scamdiscordurls = sorted(data["scamdiscordurls"])
antiscamjson.close()
...
f.write('\n\t"scamdiscordurls": ')
f.write(str(scamdiscordurls).replace("'", '"').replace("[", "[\n\t\t").replace("]", "\n\t]").replace(", ", ",\n\t\t"))
f.write('\n}')
f.close()
...
for i in range(len(scamdiscordurls)):
    for j in range(i + 1, len(scamdiscordurls)):
        if scamdiscordurls[i] == scamdiscordurls[j]:
            print("Duplicate:", scamdiscordurls[i])
